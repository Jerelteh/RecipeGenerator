            ...
            <div>
                <form action="{{ route('update.recipe', ['id' => $recipe->id]) }}" method="POST">
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="10" cols="110" required>{{ $recipe->content }}</textarea>
                    </div>
                    <button class="btn btn-primary" type="submit">Update Recipe</button>
                </form>
            </div>
            <a class="btn btn-secondary" href="{{ route('view.recipe', ['id' => $recipe->id]) }}">Cancel</a>
        </div>
    </div>
